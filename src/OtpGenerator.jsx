...
const OtpGenerator = () => {
  ...
  return (
    <div className='min-h-screen bg-gray-900 text-gray-200 flex flex-col items-center justify-center gap-6 p-6'>
      <h1 className='text-2xl font-bold text-white uppercase tracking-wide' id='otp-title'> One Time Password Generator</h1>

      {codeOTP === null ? (<h2>"Click 'Generate OTP' to get a code"</h2>)
        : (<h2>{codeOTP}</h2>)
      }
      <p className="text-md text-gray-300 font-medium mt-2" id='otp-timer' aria-live="polite">
      ...
      <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full" id='generate-otp-button' onClick={generateOTP}
      >
        Generate OTP</button>
    </div>
  )
